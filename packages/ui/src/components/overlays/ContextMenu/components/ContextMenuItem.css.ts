...
export const sprinkles = createSprinkles(tokenizedThemeColorProperties)
...
export const itemHoverable = style({
  selectors: {
    ...
    '.dark &:hover': { backgroundColor: withOpacity(COLORS['bg-700'], 0.5) }
  }
})
...
export const itemActiveDangerous = style({
  ...
  selectors: {
    '&:hover': { color: COLORS['bg-800'] },
    '.dark &:hover': { color: COLORS['bg-50'] }
  }
})
