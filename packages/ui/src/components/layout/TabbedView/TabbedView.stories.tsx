...
import { createTabbedView } from './index'
...
const TaskTab = createTabbedView({ tabs: TABS })
...
/**
 * Basic tab switching using the When component for conditional rendering. Each When
...
function FilteredTaskList() {
  const { currentTab } = TaskTab.useContext()
  ...
  return (
    <Flex direction="column" gap="sm">
      {filtered.length === 0 ? (
        ...
        filtered.map(task => <TaskItem key={task.label} label={task.label} />)
      )}
    </Flex>
  )
}
...
/**
 * Using the currentTab from useContext() to filter data. "All" shows every task,
...
const AmountTab = createTabbedView({ tabs: AMOUNT_TABS })
  ...
  render: () => (
    <AmountTab.Root>
      <Box width="100%">
        <AmountTab.Selector />
        <Box mt="lg">
          <AmountTab.When tabId="unwatched">
            <Flex direction="column" gap="sm">
...
const FilteredTab = createTabbedView({
  enabled: ['tab1', 'tab3'],
  tabs: FILTERED_TABS
})
...
export const FilteredTabs: Story = {
  render: () => (
    <FilteredTab.Root>
      <Box width="100%">
        <FilteredTab.Selector />
        <Box mt="lg">
          <FilteredTab.When tabId="tab1">
            <Flex direction="column" gap="sm">
              {['tab1'].map(id => (
...
const ColoredTab = createTabbedView({ tabs: COLORED_TABS })
    ...
    <ColoredTab.Root>
      <Box width="100%">
        ...
        <Box mt="lg">
          ...
          <ColoredTab.When tabId="warnings">
            <Card direction="row">
              <Flex align="center" gap="md">
                <Icon
                  color="yellow-500"
          ...
          <ColoredTab.When tabId="success">
            <Card direction="row">
              <Flex align="center" gap="md">
                <Icon color="green-500" icon="tabler:circle-check" size="2em" />
                <Flex direction="column" gap="xs">
