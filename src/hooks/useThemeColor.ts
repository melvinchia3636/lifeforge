...
export default function useThemeColors(): {
  ...
  const componentBg = useMemo(() => {
    if (bgImage !== '') return 'bg-bg-50/50 backdrop-blur-sm dark:bg-bg-900/50'
    return 'bg-bg-50 dark:bg-bg-900'
  }, [bgImage])

  const componentBgWithHover = useMemo(() => {
    if (bgImage !== '') {
      return 'bg-bg-50/50 backdrop-blur-sm dark:bg-bg-900/50 hover:bg-bg-100/50 dark:hover:bg-bg-800/50 transition-all'
    }
