...
function MasterSearchBar(): React.ReactElement {
  ...
  return (
    <div className="relative z-50">
      ...
      />
      {debouncedMasterSearchQuery.length >= 3 && (
        <div className="absolute z-50 mt-4 flex h-96 w-full flex-col rounded-md border-2 border-bg-800 bg-bg-900">
          <APIComponentWithFallback data={searchResults}>
            {searchResults => (
              <Scrollbar className="flex-1">
                <div className="flex-1 divide-y divide-bg-800">
                  {searchResults.length > 0 ? (
                    searchResults.map(airport => (
                      <Link
                        key={airport.id}
                        to={`/aviation/airports/${airport.continentCode}/${airport.country.code}/${airport.region.code}/${airport.id}`}
                        className="flex w-full items-center justify-between p-4 px-6 transition-all hover:bg-bg-200/50 dark:hover:bg-bg-800/50"
                      >
