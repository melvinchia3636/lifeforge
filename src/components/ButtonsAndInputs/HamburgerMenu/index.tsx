...
function getColorClass(lighter?: boolean): string {
  if (lighter === true) {
    ...
    return 'text-bg-500 hover:bg-bg-100 hover:text-bg-800 dark:hover:text-bg-100 dark:hover:bg-bg-700/30'
  }
}
      ...
      >
        <TransitionChild>
          <MenuItems
            ...
            className={`mt-2 ${
              customWidth ?? 'w-48'
            } z-[9991] overflow-hidden overscroll-contain rounded-md border border-bg-200 bg-bg-100 shadow-lg outline-none transition duration-100 ease-out focus:outline-none data-[closed]:scale-95 data-[closed]:opacity-0 dark:border-bg-700 dark:bg-bg-800`}
          >
            {children}
          </MenuItems>
        </TransitionChild>
