...
3. Edit `env/.env.docker` and set the required variables:
...
Usually, installing and removing modules through ForgeCLI will automatically sync the schemas. If you have modified the schemas manually, you will need to run `bun forge db generate-schemas` to update the schemas.
</Alert>
