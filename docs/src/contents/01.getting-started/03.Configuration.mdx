...
where the `&lt;email&gt;` and `&lt;password&gt;` are the email and password for the admin user you want to create.

<Alert className="mt-6" type="note">
  When initializing the database, the dotenv file will be created automatically
  in the `env` directory if it doesn't exist. If it already exists,
...
- Update the `.env` file with the admin email and password (a new one will be created based on the `.env.example` file if it doesn't exist).
...
There are only several environment variables that need to be set up for the system to work properly. These variables are stored in a dotenv file located in the `env` directory. A default dotenv file is provided in the `env/.env.example` file. You can copy this file to `env/.env.local` and modify the values as needed.
