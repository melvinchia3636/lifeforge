...
const myHandler = forgeController
  .query()
  .description({
    en: "This is a sample query handler",
    ms: "Ini adalah pengendali pertanyaan sampel",
    'zh-CN': '这是一个示例查询处理程序',
    'zh-TW': '這是一個範例查詢處理程式'
  })
  .input({
...
Paired with the `forgeAPI` system in the frontend, you can call the API like this:
...
For example, we have a module for managing personal finances, which includes handlers for managing accounts and transactions. Inside the `src/Finances` directory, we have the following structure:
...
In `accounts.ts`, we define the API handlers for managing accounts:
...
Similarly, in `transactions.ts`, we define the respective API handlers for managing transactions.

Then, in `index.ts`, we import the routers from `accounts.ts` and `transactions.ts` and group them together:
...
Finally, in the routing file located at `src/core/routes/app.routes.ts`, we import the router from `src/Finances/index.ts` and register it into the Express.js application:
...
- `userData`: The authenticated user's data from PocketBase (available for authenticated endpoints).
...
<Alert className="mt-6" type="note">
  The `userData` property contains the authenticated user's information, including their `id`, `username`, `email`, and other PocketBase user fields. This is available for all authenticated endpoints (i.e., those that don't use `.noAuth()`).
</Alert>
...
```typescript
const createWithAttachments = forgeController
  .mutation()
  ...
  .callback(async ({ body, media, pb }) => {
    return pb.create
      .collection('items')
      .data({
        ...body,
        ...(await getMedia('cover', media.cover)),
        ...(await getMedia('attachments', media.attachments))
      })
      .execute();
  });
```
...
Errors thrown inside the `.callback()` function are automatically caught and handled by the `forgeController`. You don't need to wrap your code in try-catch blocks for most cases.
...
- `.description()`: This method is used to provide a brief
  ...
  handler. It can accept either a plain string or an object with localized descriptions:
  ```typescript
  // Simple string
...
- `.middleware()`: This method is used to add custom Express.js
