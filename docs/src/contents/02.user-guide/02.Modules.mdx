...
```bash
bun run forge modules install <module-name>
```
...
```bash
bun run forge modules install lifeforge--calendar lifeforge--todo-list lifeforge--idea-box
```
...
```bash
bun run forge modules upgrade lifeforge--calendar
```

**Check all modules for updates:**
```bash
bun run forge modules upgrade
```
...
```bash
bun run forge modules uninstall <author>--<module-name>
```
...
```bash
bun run forge modules uninstall lifeforge--calendar
```
...
```bash
bun run forge modules uninstall lifeforge--calendar lifeforge--todo-list lifeforge--idea-box
```
...
```bash
bun run forge modules uninstall <module-name>
bun run forge modules install <full-module-name>
```
...
3. Try running `bun run forge modules list` to verify the module is installed
...
|---------------------|---------|---------|
| List installed modules | `bun run forge modules list` | `ls` |
| Install a module | `bun run forge modules install <author>--<module>` | `i`, `add` |
| Uninstall a module | `bun run forge modules uninstall <author>--<module>` | `un`, `rm`, `remove` |
| Update a specific module | `bun run forge modules upgrade <author>--<module>` | `up`, `u` |
| Check all modules for updates | `bun run forge modules upgrade` | `up`, `u` |
...
<Alert className="mt-6" type="tip">
  **Pro tip:** Use aliases to save time! For example:
  - `bun run forge modules i lifeforge--calendar` instead of `install`
  - `bun run forge modules rm lifeforge--calendar` instead of `uninstall`
  - `bun run forge modules up` instead of `upgrade`
</Alert>
