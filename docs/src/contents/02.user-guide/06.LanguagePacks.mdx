...
| Command | Description |
|---------|-------------|
| `pnpm forge locales list` | List all installed language packs |
| `pnpm forge locales add <lang>` | Install a new language pack |
| `pnpm forge locales remove <lang>` | Remove an installed language pack |
| `pnpm forge locales validate <lang>` | Validate a language pack structure |
...
```bash
pnpm forge locales add <package-name>
```

For example, to install Japanese:

```bash
pnpm forge locales add lifeforge--lang-ja
```
...
```bash
pnpm forge locales remove <package-name>
```

For example, to remove Simplified Chinese:

```bash
pnpm forge locales remove melvinchia3636--lang-zh-CN
```
...
- Restart the development server (`pnpm forge dev`)
