...
Currently, all official LifeForge modules (those you have seen living inside the monorepo before) are hosted on GitHub under the [LifeForge App](https://github.com/lifeforge-app) organization. Each module will be in its own repository, with the name formatted as `lifeforge-module-\<moduleName\>`. 
...
In the very near future, there will be an official LifeForge module registry / marketplace website, namely Forgistry, where you can browse and search for modules easily.
</Alert>
</section>
...
```bash
bun run forge modules add lifeforge-app/lifeforge-module-ideaBox
```
...
Uninstalling a module is just as easy as installing one. You can use the `ForgeCLI` tool to remove a module from your LifeForge instance. When you want to remove the module, the name you will use is simply the `\<moduleName\>` part.
