...
- **UI**: Fixed some silly but fatal bugs related to <code>useCallback</code>.
- **UI**: Improved styling for different state of button components.
- **UI**: Moved global modal manager components and mechanism to <code>lifeforge-ui</code>.
- **UI**: Mega performance optimization for <code>FormModal</code> component by removing unnecessary re-renders.
