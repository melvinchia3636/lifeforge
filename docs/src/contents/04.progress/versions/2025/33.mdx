...
- Refactored API Builder to adapt to new SSO app SDK (Notably, all the authentication flows are now packed in an integrated component called `SSOAppMainView`).
...
- Added the new `RRuleInput` component from `lifeforge-ui` into the Calendar module, now the modify modal is finally working properly again.
- Implemented RRule string parser to parse RRule string back into `RRuleInput`, and now recurring events can now be updated (Originally, they could not be modified at all after creation).

### 🎨 UI Components & Design System

- Migrated from manually implementing `Listbox` component from `@headlessui/react` to using `Listbox` component from `lifeforge-ui`. See [commit](https://github.com/lifeforge-app/lifeforge/commit/5e6ea6076669e2d41ef3ec409022dc560f32ee5e) (Commits are still not properly staged at this point in time, apologies).
- Removed all confusing `lighter` props from various input components and unified their background colors.
- Introduced a new props into `ListboxOption` component: `renderColorAndIcon`, which allows for custom rendering of the option's color and icon.
...
- <Commit hash="27cea6d71214f8c2b15123a24c6adb5bd19e662e" />: Updated
  `ConfirmationModal` component to accept custom button component for
  confirmation button.
...
- Renamed the `Previous` button to `Prev`.
- Added back icon in module header in mobile view.
- Value will now be default to 0 when the `NumberInput` is empty.
- <Commit hash="78575fdefafb4e644b9b6b58499fdd114bb20678" />: Added
  `RRuleInput` component to `lifeforge-ui`, allowing users
  to create and edit recurrence rules for events.
- `SliderInput` component now handles decimal values correctly.
...
- Replaced `z.infer` with `z.input` in the forgeAPI type inference system for more accurate type safety.
...
- <Commit hash="45168157fcca480c7112e30a8e4449f22fb79fc8" />: Bumped the version
  of storybook in `lifeforge-ui` from version 8 to version 9, added
  `autodocs` feature for automatic documentation generation.
...
- <Commit hash="2c4802c2d1709979bb948b855b6365dfb2445214" />: Added docstring to
  `FormBuilder` class.
- Added i18n support for the storybook of `lifeforge-ui`.
