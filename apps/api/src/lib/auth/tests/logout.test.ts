...
describe('POST /auth/logout', () => {
  ...
  it('returns 401 with invalid cookie', async () => {
    const res = await forgeAPI.auth.logout.mutateRaw(
      undefined,
  ...
  it('logged-out cookie cannot be reused', async () => {
    ...
    await forgeAPI.auth.logout.mutateRaw(
      undefined,
      { raw: true, headers: { Cookie: cookies } }
    )

    const after = await forgeAPI.auth.refresh.mutateRaw(
      undefined,
      { raw: true, headers: { Cookie: cookies } }
    )
  ...
  it('all tokens in the family are invalidated after logout', async () => {
    ...
    const refreshRes = await forgeAPI.auth.refresh.mutateRaw(
      undefined,
      { raw: true, headers: { Cookie: cookies } }
    )
    ...
    const after1 = await forgeAPI.auth.refresh.mutateRaw(
      undefined,
      { raw: true, headers: { Cookie: cookies } }
    )
    const after2 = await forgeAPI.auth.refresh.mutateRaw(
      undefined,
      { raw: true, headers: { Cookie: rotatedCookie } }
    )
  ...
  it('double logout is safe', async () => {
    ...
    const again = await forgeAPI.auth.logout.mutateRaw(
      undefined,
      { raw: true, headers: { Cookie: cookies } }
    )
